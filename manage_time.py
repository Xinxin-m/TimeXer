...
'''
    TODO: 
        ...
        - before training: change name of saved file model_name_4_saving and change it in optimization>ff_scenario_time.py

'''

# chunksize etc. imported from file optimization > ff_scenario_time.py
class RpodDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Select the chunk
        dt = self.data['data_param']['time_discr'][ix].item() # time_discr = delta_t = 0.5 by default for this project
        final_time = self.data['data_param']['final_time'][ix].item() # final time
        T = round(final_time/dt) # total time steps

        if T > self.chunksize:
            # select an interval of 'chunksize' time steps uniformly in [0,T)
            max_start_index = max(0, T - self.chunksize)
            start_index = np.random.randint(0, max_start_index + 1)
            time_indexes = np.arange(start_index, start_index + self.chunksize)
        else: 
            time_indexes = np.arange(0, self.chunksize)

        ''' # select first chunk or last chunk, chunksize = 100 by default (defined in ff_scenario_time.py)
        ...
        if self.target == False:
            if not self.mdp_constr:
                return states, actions, rtgs, goal, timesteps, attention_mask, dt, time_sec, ix
            else:
                ctgs = torch.stack([self.data['ctgs'][i, time_indexes]
                            for i in ix]).view(self.chunksize, 1).float()
                ttgs = torch.stack([self.data['ttgs'][i, time_indexes]
                            for i in ix]).view(self.chunksize, 1).float()
                return states, actions, rtgs, ctgs, ttgs, goal, timesteps, attention_mask, dt, time_sec, ix
        else:
            ...
            if not self.mdp_constr:
                return states, actions, rtgs, goal, target_states, target_actions, timesteps, attention_mask, dt, time_sec, ix
            else:
                ctgs = torch.stack([self.data['ctgs'][i, time_indexes]
                            for i in ix]).view(self.chunksize, 1).float()
                ttgs = torch.stack([self.data['ttgs'][i, time_indexes]
                            for i in ix]).view(self.chunksize, 1).float()
                return states, actions, rtgs, ctgs, ttgs, goal, target_states, target_actions, timesteps, attention_mask, dt, time_sec, ix
    
    def getix(self, ix):
        ix = [ix]
        # Select the chunk
        dt = self.data['data_param']['time_discr'][ix].item()
        ...
        '''
        time_index_final = round(final_time/dt)
        if time_index_final > self.chunksize:
            if np.random.choice([0,1],1) == 0:
                time_indexes = np.arange(0, self.chunksize)
            else:
...
def get_DT_model(model_name, train_loader, eval_loader):
    ...
    model = AutonomousFreeflyerTransformer_pred_time(config) # Xin: used this for checkpoint_ff_time_90_random
